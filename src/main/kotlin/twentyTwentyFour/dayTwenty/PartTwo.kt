...
class PartTwo {
    companion object {
        fun execute(lines: List<String>) {
            ...
            println("NOT WORKING, DO NOT USE")

            /*val minStepsWithoutCheat = aStar(map, start, end, Pair(false, AtomicInteger(0))).first
        ...
        private fun aStar(
            ...
            cheat: Pair<Boolean, AtomicInteger>
            ...
            while (openSet.isNotEmpty()) {
                ...
                if (current.point == end) {
                    return Pair(current.steps, current.point)
                }

                val canCheat = cheat.first && cheat.second.decrementAndGet() >= 0
                if(cheat.first && !canCheat) {
                    return Pair(current.steps, current.point)
                }
                ...
                for (direction in Direction.values()) {
                    ...
                    if (isValid(map, nextPoint) || canCheat) {
            ...
            while (queue.isNotEmpty()) {
                ...
                for (direction in Direction.values()) {
                    ...
                    if (isValid(map, nextPoint)) {
                        ...
                        repeat(20) {
                            val cheatSteps = aStar(map, current.point, end, Pair(true, AtomicInteger(it+1)))
                            if (cheatSteps.first != Int.MAX_VALUE) {
                                cheatPaths.add(cheatSteps.first + current.steps + 2)
                                breachedWallsWithCheat.add(Pair(nextPoint, cheatSteps.second!!))
                            }
                        }
                    }
                }
