...
class PartTwo {
    companion object {
        ...
        private fun evaluateHistoryValue(history: List<List<Long>>, valueOfSequenceBefore: Long, index: Int): Long {
            val firstNumberOfHistorySequence = history[index][0]
            return if(0 == index) {
                firstNumberOfHistorySequence - valueOfSequenceBefore
            } else {
                evaluateHistoryValue(history,  firstNumberOfHistorySequence - valueOfSequenceBefore, index - 1)
            }
        }
    }
}
